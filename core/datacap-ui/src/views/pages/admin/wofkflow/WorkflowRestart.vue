...
defineOptions({ name: 'WorkflowRestart' })
...
const onSubmit = () => {
  if (props.info) {
    loading.value = true
    WorkflowService.restart(props.info.code!)
                   .then((response) => {
                     if (response.status) {
                       message.success(t('workflow.tip.restartSuccess').replace('$VALUE', String(props.info?.name)))
                       onCancel()
                     }
                     else {
